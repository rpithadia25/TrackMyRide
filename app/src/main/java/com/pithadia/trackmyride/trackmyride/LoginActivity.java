...
public class LoginActivity extends AppCompatActivity {
    ...
    private class SampleLoginCallback implements LoginCallback {
        ...
        public void onAuthorizationCodeReceived(@NonNull String authorizationCode) {
            Toast.makeText(LoginActivity.this, getString(R.string.authorization_code_message, authorizationCode),
                    Toast.LENGTH_LONG)
                    .show();
        }
    }
                ...
                .enqueue(new Callback<UserProfile>() {
                    ...
                    public void onResponse(Call<UserProfile> call, Response<UserProfile> response) {
                        if (response.isSuccessful()) {
                            Toast.makeText(LoginActivity.this, getString(R.string.greeting, response.body().getFirstName()), Toast.LENGTH_LONG).show();
                            startLocationActivity();
                        } else {
                            ApiError error = ErrorParser.parseError(response);
                            Toast.makeText(LoginActivity.this, error.getClientErrors().get(0).getTitle(), Toast.LENGTH_LONG).show();
                        }
                    }
